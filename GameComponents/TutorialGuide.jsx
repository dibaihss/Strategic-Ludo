...
const getStepLayout = (step, isSmallScreen) => {
  ...
  if (step === 1) {
    return {
      popup: {
        top: isSmallScreen ? 78 : 110,
        left: isSmallScreen ? 14 : undefined,
        right: isSmallScreen ? 14 : 36,
      },
      target: {
  ...
  if (step === 3) {
    return {
      popup: {
        top: isSmallScreen ? '44%' : '41%',
        left: isSmallScreen ? 14 : undefined,
        right: isSmallScreen ? 14 : 26,
      },
      target: {
...
export default function TutorialGuide({ visible, step, onSkip }) {
  ...
  const steps = [
    ...
    },
    {
      ...
      hint: texts.tutorialStep5Hint || 'Complete one turn to finish this tutorial.',
    },
  ];
  ...
  const targetStyle = useMemo(() => {
    if ((safeStep !== 0 && safeStep !== 1) || !dynamicAnchor) {
      return layout.target;
    }

    const highlightPadding = safeStep === 1 ? 6 : 8;
    return {
